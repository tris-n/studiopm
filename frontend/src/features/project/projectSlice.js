...
// // updateProject
export const updateProject = createAsyncThunk('projects/updateProject', async (projectUpdate, thunkAPI) => {
	
	try {

		const { projectId, update } = projectUpdate

		const token = thunkAPI.getState().user.user.token
		return await projectService.updateProject(projectId, update, token)
