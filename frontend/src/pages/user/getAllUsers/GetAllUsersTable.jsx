...
// Material UI
import { Avatar, Box, Button, Card, Checkbox, FormControl, IconButton, InputLabel, MenuItem, OutlinedInput, Paper, Popover, Select, Stack, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, TableSortLabel, Toolbar, Tooltip, Typography } from "@mui/material"
			...
			{numSelected > 0 ? (
				...
				<FormControl sx={{width: "250px", mr: 2}}>
					...
					>
						...
						<MenuItem value={"producer"}>Producers</MenuItem>
						<MenuItem value={"studio head"}>Studio Heads</MenuItem>
...
const GetAllUsersTable = () => {
	...
	const handleMenuEditClick = () => {
		navigate(`/crew/update/${currentId}`)
	}
	...
	const handleDeleteUsers = () => {
		...
		setIsOpen(false)
			...
			dispatch(deleteUsers(selected))
				.unwrap()
				.then(() => {
					toast.success(`Successfully deleted the crew.`)
					setSelected([])
					setPage(0)
					dispatch(getAllUsers())
				})
									...
									>
										...
										>
											<Stack direction="row" alignItems="center">
													<Avatar src={`/images/avatars/${user.name.slice(0,1).toLowerCase()}.png`} alt="" width="45px" sx={{mr: 1}}/>
													<strong>{user.name}</strong>
												</Stack>
										</TableCell>

										<TableCell align="left" onClick={(event) => handleRowClick(event, user._id)}>{user.role}</TableCell>
										<TableCell align="left" onClick={(event) => handleRowClick(event, user._id)}>{user.email}</TableCell>
										<TableCell align="left" onClick={(event) => handleRowClick(event, user._id)}>{user.projectsCount}</TableCell>
										<TableCell align="right">
											<IconButton onClick={(event) => handleOpenMenu(event, user._id)}>
												<MoreVertIcon />
											</IconButton>
										</TableCell>
